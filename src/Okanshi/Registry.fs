...
type OkanshiMonitorRegistry() =
    ...
    let register' (monitor : IMonitor) =
        ...
        if not <| monitors.ContainsKey(hash) then
            monitors.Add(hash, monitor) |> ignore
    ...
    /// Get or add a new registration
    member __.GetOrAdd<'a when 'a :> IMonitor>(config, factory : Func<MonitorConfig, 'a>) = Lock.lockWithArg syncRoot (config, factory) getOrAdd'
